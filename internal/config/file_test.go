...
`
	...
	if len(cfg.Databases) != 1 || cfg.Databases[0].URL != "postgres://localhost/testdb" {
		t.Errorf("Databases[0].URL: got %#v", cfg.Databases)
	}
	if cfg.Interval != 120 {
		t.Errorf("Interval: got %d", cfg.Interval)
	}
	if cfg.Databases[0].ThresholdLevels != "70,85,95" {
		t.Errorf("ThresholdLevels: got %q", cfg.Databases[0].ThresholdLevels)
	}
...
func TestFromFile_LegacyDBRejected(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "pgwd.conf")
	content := `
client: monitor-01
...
`
	...
	if err == nil {
		t.Fatal("expected error for removed db: key")
	}
	if loaded {
		t.Error("expected loaded=false on error")
	}
	if !strings.Contains(err.Error(), "db:") {
		t.Errorf("error %q should mention db:", err)
	}
}
...
`
	...
	if p.StaleAge != 300 {
		t.Errorf("prod StaleAge: got %d", p.StaleAge)
	}
	if p.DefaultThresholdPercent != 80 {
		t.Errorf("prod DefaultThresholdPercent: got %d", p.DefaultThresholdPercent)
	}
	a := cfg.Databases[1]
	if a.StaleAge != 600 {
		t.Errorf("analytics StaleAge: got %d", a.StaleAge)
	}
	if a.DefaultThresholdPercent != 70 {
		t.Errorf("analytics DefaultThresholdPercent: got %d", a.DefaultThresholdPercent)
	}
	if a.ThresholdLevels != "75,85,95" {
		t.Errorf("analytics ThresholdLevels (default): got %q", a.ThresholdLevels)
	}
}
