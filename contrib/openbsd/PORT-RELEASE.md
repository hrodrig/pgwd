...
ver=$(make -V PKGNAME | sed 's/^pgwd-//')    # e.g. 1.0.1
dist=$(make -V DISTFILES)                     # e.g. pgwd_v1.0.1_openbsd_amd64.tar.gz
...
Set **`pgwd_version`** in **`testing/platforms/inventory/hosts.yml`** to match the release (e.g. **`1.0.1`**) or use **`pgwd_local_package`** pointing at **`dist/pgwd_v...tar.gz`**.
...
```sh
doas sh /tmp/pgwd-port/test-install-from-dist.sh /tmp/pgwd_v1.0.1_openbsd_amd64.tar.gz /tmp/pgwd-port
```
