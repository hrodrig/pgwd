...
- **Config:** Config file (YAML) at `/etc/pgwd/pgwd.conf` or `-config` / `PGWD_CONFIG`. Single DB: `db`; multi-DB: `databases` array (daemon mode; direct URLs only; kube.postgres not supported). When the file loads, env vars are ignored; otherwise `ApplyDefaults` + `ApplyEnv` (`PGWD_*`). CLI flags always override. Structure: `db`, `databases`, `kube`, `notifications` (loki, slack). `client` required when using `databases`. See `internal/config`, `internal/config/file.go`, `contrib/pgwd.conf.example`.
