...
- **Root build on FreeBSD:** Real rules live in **`GNUmakefile`** (GNU Make). Root **`Makefile`** is a **BSD Make** stub that forwards to **`gmake`** (`pkg install gmake`); uses **`all`** + **`.DEFAULT`** because BSD Make does not apply **`%:`** to arbitrary targets. **`.gitignore`** whitelists **`GNUmakefile`**.
